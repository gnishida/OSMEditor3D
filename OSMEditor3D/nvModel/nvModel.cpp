...
namespace nv {
	...
	void Model::rescale( float radius) {
	 ...
	 void Model::scalePerNumber( float factor){
		 if ( _positions.empty())
			return;
	 ...
	 void Model::scalePerNumber( float factorX,float factorY,float factorZ){
		 if ( _positions.empty())
			return;
	 ...
	 void Model::centerXYzeroZAndSetMaxSize(float maxSize){
		 vec3f minVal, maxVal;
		 computeBoundingBox(minVal, maxVal);
		 printf("Min %f %f %f Max %f %f %f\n",minVal.x,minVal.y,minVal.z,maxVal.x,maxVal.y,maxVal.z);
		 vec3f size=vec3f(abs(maxVal.x-minVal.x),abs(maxVal.y-minVal.y),abs(maxVal.z-minVal.z));
		 //center and scale
		 float scaleX=maxSize/size.x;
		 float scaleY=maxSize/size.y;
		 float scaleZ=maxSize/size.z;
		 float scale=std::min(std::min(scaleX,scaleY),scaleZ);
		 for ( vector<float>::iterator pit = _positions.begin(); pit < _positions.end(); pit += _posSize) {
			 pit[0] = (pit[0]-minVal.x-size.x/2.0f)*scale;
			 pit[1] = (pit[1]-minVal.y-size.y/2.0f)*scale;
			 pit[2] = (pit[2]-minVal.z)*scale;//-size.z/2.0f;
		 }
		 computeBoundingBox(minVal, maxVal);
		 size=vec3f(abs(maxVal.x-minVal.x),abs(maxVal.y-minVal.y),abs(maxVal.z-minVal.z));
